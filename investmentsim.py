...
if st.session_state.username:
    ...
    if st.sidebar.button("Login / Signup"):
        if username_input:
            ...
            st.rerun()  # refresh page so login form disappears and logout appears
...
# --- TRADE SECTION ---
if st.session_state.username:  # only allow trading after login
...
if portfolio:
    ...
    st.write(df)
    st.write(f"Total Portfolio Value (Stocks Only): ₹{total_value:,.2f}")
else:
